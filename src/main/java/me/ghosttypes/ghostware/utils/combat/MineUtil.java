...
public class MineUtil {
    ...
    public static void handlePacketMine(BlockPos targetBlock, MineMode mineMode, boolean rotate, int rotatePrio) {
        ...
        if (pick.found()) {
            canHandle = true;
            waitTime = (long) getBlockBreakingSpeed(pick.getSlot(), mc.world.getBlockState(targetBlock));
            int currentSlot = mc.player.getInventory().selectedSlot;
            mc.player.networkHandler.sendPacket(new UpdateSelectedSlotC2SPacket(pick.getSlot()));
