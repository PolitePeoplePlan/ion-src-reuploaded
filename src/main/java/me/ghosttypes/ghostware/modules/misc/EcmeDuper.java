...
public class EcmeDuper extends Module {
    ...
    private final Setting<Boolean> announce = sgGeneral.add(new BoolSetting.Builder().name("announce").description("Announce when you dupe items.").defaultValue(false).build());
    private final Setting<Integer> delay = sgGeneral.add(new IntSetting.Builder().name("dupe-delay").description("The delay in seconds between duping.").defaultValue(30).min(0).build());
    ...
    private int timer, dupeTimes, dupedItems;

    public EcmeDuper() {
        super(Categories.Misc, "ecme-duper", "Auto dupe for ecme. Disable with esc.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (timer <= 0) {
            timer = MathUtil.intToTicks(delay.get());
            //timer = delay.get();
        } else {
            timer--;
            return;
        ...
        if (slots == -1) {
            ...
            if (isDupeTime()) {
                if (!slotsToThrow.isEmpty()) {
                    if (faceDown.get()) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(mc.player.getYaw(), 90F, mc.player.isOnGround()));
                    for (int i : slotsToThrow) InvUtils.drop().slotId(i);
                    slotsToThrow.clear();
                } else {
                    for (int i = 2; i < getDupeSize() + 1; i++) slotsToThrow.add(i);
                }
            } else {
                mc.player.closeHandledScreen();
            ...
            if (!slotsToMove.isEmpty()) {
                ...
                info("Doing dupe #" + dupeTimes + " | Total Shulkers duped: " + dupedItems);
                if (announce.get()) Wrapper.sendMessage("I just duped " + slotsToMove.size() + " " + InvUtil.getItemNameBySlot(slotsToMove.get(0)) + " thanks to Ghostware!");
                slotsToMove.clear();
            }
        }
    }
