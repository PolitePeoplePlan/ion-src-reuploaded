...
public class AnchorAura extends Module {
    ...
    public AnchorAura() {
        super(Categories.Combat, "anchor-aura", "Anchor Aura v2.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        if (mc.world.getDimension().isRespawnAnchorWorking()) {
            notify("You are in the Nether... disabling.");
            toggle();
            return;
        }
        ...
        boolean rotate = rotationMode.get() == RotationMode.Both || rotationMode.get() == RotationMode.Break;
        ...
        if (antiStuck.get() && !sentAntiStuck) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && BlockHelper.getBlock(target.getBlockPos().up(2)) == Blocks.GLOWSTONE) {
                ...
                if (pick.found()) {
                    ...
                    AutomationUtils.doPacketMine(target.getBlockPos().up(2), rotate, rotatePrio.get());
                    sentAntiStuck = true;
                    return;
                }
            }
        ...
        if (breakSelfTrap.get() && !sentTrapMine) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && AutomationUtils.isTrapBlock(target.getBlockPos().up(2))) {
                FindItemResult pick = InvUtil.findPick();
                //TODO: update this to use MineHelper
                if (pick.found()) {
                    Wrapper.updateSlot(pick.getSlot());
        ...
        if (breakDelayLeft >= breakDelay.get()) {
            ...
            if (breakPos != null) {
                ...
                switch (rotationMode.get()) {
                    case None -> breakAnchor(breakPos, anchor, glowStone);
                    case Both, Break -> Rotations.rotate(Rotations.getYaw(breakPos), Rotations.getPitch(breakPos), 50, () -> breakAnchor(breakPos, anchor, glowStone));
                }
    ...
    private void notify(String msg) {
        switch (notifyMode.get()) {
            case Chat -> info(msg);
            case Hud -> NotificationsHUD.addNotification(msg);
        }
    }
