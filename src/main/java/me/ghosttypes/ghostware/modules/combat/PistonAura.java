...
public class PistonAura extends Module {
    ...
    private final Setting<Integer> range = sgGeneral.add(new IntSetting.Builder().name("range").description("Range for working module.").defaultValue(5).min(0).sliderMax(7).build());
    private final Setting<Integer> piston_place_delay = sgGeneral.add(new IntSetting.Builder().name("piston-place-delay").description("Delay for placing piston.").defaultValue(1).min(0).sliderMax(10).build());
    private final Setting<Integer> crystal_place_delay = sgGeneral.add(new IntSetting.Builder().name("crystal-place-delay").description("Delay for placing crystal.").defaultValue(1).min(0).sliderMax(10).build());
    private final Setting<Integer> crystal_attack_delay = sgGeneral.add(new IntSetting.Builder().name("crystal-attack-delay").description("Delay for attacking crystals.").defaultValue(1).min(0).sliderMax(10).build());
    private final Setting<Boolean> fast_mode = sgGeneral.add(new BoolSetting.Builder().name("fast-mode").description("Tries to speed up module.").defaultValue(true).build());

    public PistonAura() {
        super(Categories.Combat, "piston-aura", "Automatically pushing crystals into enemy by piston.");
    }
    ...
    private boolean checkSide(BlockPos pos, Direction direction) {
        ...
        if (bs(pos.up(), Blocks.AIR) ||
            ...
            bs(pos.up(), Blocks.PISTON_HEAD)) sideInt += 1;
        switch (direction) {
            case WEST -> {
    ...
    public BlockPos findButton(int range) {
        for (BlockPos blockPos : BlockHelper.getSphere(mc.player.getBlockPos(), range, range))
